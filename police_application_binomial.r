...
for (p in sort(unique(dados$first_trained))[-1]){
  ...
  F00_hat_meu=pbinom(sum(Y_10), size = length(Y_10),  prob = mean(Y_00))
  F_inver_01_hat_meu=qbinom((F00_hat_meu), size = length(Y_10),  prob = mean(Y_01))

  ATT_meu=(mean(Y_11)-mean(F_inver_01_hat_meu)/length(Y_10))
  ...
  #####################################
  Y_10=mean(minha_base$complaints_BEFORE[minha_base$TREATED_GROUP==1])
  Y_11=mean(minha_base$complaints_AFTER[minha_base$TREATED_GROUP==1])
  Y_00=mean(minha_base$complaints_BEFORE[minha_base$TREATED_GROUP==0])
  Y_01=mean(minha_base$complaints_AFTER[minha_base$TREATED_GROUP==0])
  
  
  G_Ybar_11_t4=log(Y_10/(1+Y_10))-log(Y_00/(1+Y_00))+log(Y_01/(1+Y_01))
  ATT_WOOLD=mean(Y_11)-(exp(G_Ybar_11_t4)/(1+exp(G_Ybar_11_t4)))
