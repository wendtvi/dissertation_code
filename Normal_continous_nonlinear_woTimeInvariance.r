  ...
  for(p in 1:N){
    ...
    for (k in 1:TT){
      for(i in 1:nrow(matriz_estado_naotratamento)){
        if (k<q && matriz_estado_naotratamento[i,TT+1]==0) matriz_estado_naotratamento[i,k]=as.numeric(-1+(Z_cov_controle[i,k]-1)*5/2-2*D[i]+(Z_cov_controle[i,k]-1)*D[i]/4+rnorm(1,rnorm(1,0,1)+Z_cov_controle[i,k]*1/k,1)+ct_0_controle_pre[i])
        if (k>=q&& matriz_estado_naotratamento[i,TT+1]==0) matriz_estado_naotratamento[i,k]=as.numeric(-1+(Z_cov_controle[i,k]-1)*5/2-2*D[i]+(Z_cov_controle[i,k]-1)*D[i]/4+rnorm(1,rnorm(1,0,1)+Z_cov_controle[i,k]*1/k,1)+ct_0_controle_pos[i])
        if (k<q && matriz_estado_naotratamento[i,TT+1]==1) matriz_estado_naotratamento[i,k]=as.numeric(-1+(Z_cov_tratados[i,k]-1)*5/2-2*D[i]+(Z_cov_tratados[i,k]-1)*D[i]/4+rnorm(1,rnorm(1,0,1)+Z_cov_tratados[i,k]*1/k,1)+ct_0_trat_pre[i])
        if (k>=q&& matriz_estado_naotratamento[i,TT+1]==1) matriz_estado_naotratamento[i,k]=as.numeric(-1+(Z_cov_tratados[i,k]-1)*5/2-2*D[i]+(Z_cov_tratados[i,k]-1)*D[i]/4+rnorm(1,rnorm(1,0,1)+Z_cov_tratados[i,k]*1/k,1)+ct_0_trat_pos[i])
    ...
    for (k in 1:(ncol(matriz_X_estrela)-1)){
      for (i in 1:nrow(matriz_X_estrela)){
        if (k<q && matriz_X_estrela[i,TT+1]==0) matriz_X_estrela[i,k]=as.numeric(-1+(Z_cov_controle[i,k]-1)*5/2-2*D[i]+(Z_cov_controle[i,k]-1)*D[i]/4+rnorm(1,rnorm(1,0,1)+Z_cov_controle[i,k]*1/k,1)+ct_0_controle_pre[i])
        if (k>=q&& matriz_X_estrela[i,TT+1]==0) matriz_X_estrela[i,k]=as.numeric(-1+(Z_cov_controle[i,k]-1)*5/2-2*D[i]+(Z_cov_controle[i,k]-1)*D[i]/4+rnorm(1,rnorm(1,0,1)+Z_cov_controle[i,k]*1/k,1)+ct_0_controle_pos[i])
        if (k<q && matriz_X_estrela[i,TT+1]==1) matriz_X_estrela[i,k]=as.numeric(-1+(Z_cov_tratados[i]-1)*5/2-2*D[i]+(Z_cov_tratados[i,k]-1)*D[i]/4+rnorm(1,rnorm(1,0,1)+Z_cov_tratados[i,k]*1/k,1)+ct_0_trat_pre[i])
        if (k>=q&& matriz_X_estrela[i,TT+1]==1) matriz_X_estrela[i,k]=as.numeric(-1+(Z_cov_tratados[i]-1)*5/2-2*D[i]+(Z_cov_tratados[i,k]-1)*D[i]/4+rnorm(1,rnorm(1,0,1)+Z_cov_tratados[i,k]*1/k,1)+ct_0_trat_pos[i])
        if (k==4 && matriz_X_estrela[i,TT+1]==1)matriz_X_estrela[i,k]=as.numeric(-1+0.5+(Z_cov_tratados[i]-1)-2*D[i]+rnorm(1,rnorm(1,0,1)+Z_cov_tratados[i,k]*1/k,1)+ct_inf_trat_pos[i])
        if (k==5 && matriz_X_estrela[i,TT+1]==1)matriz_X_estrela[i,k]=as.numeric(-1+0.5+(Z_cov_tratados[i]-1)-2*D[i]+0.2*f5[i]+rnorm(1,rnorm(1,0,1)+Z_cov_tratados[i,k]*1/k,1)+ct_inf_trat_pos[i])
        if (k==6 && matriz_X_estrela[i,TT+1]==1)matriz_X_estrela[i,k]=as.numeric(-1+0.5+(Z_cov_tratados[i]-1)-2*D[i]+0.2*f5[i]+0.3*f6[i]+rnorm(1,rnorm(1,0,1)+Z_cov_tratados[i,k]*1/k,1)+ct_inf_trat_pos[i])
      }
    }
    ...
    gamma4_hat=mean(X_vetor_1_inf[,q])-(exp((G_Ybar_01_t4-G_Ybar_00_t3+G_Ybar_10_t3)))
    gamma5_hat=mean(X_vetor_1_inf[,q+1])-(exp((G_Ybar_01_t5-G_Ybar_00_t3+G_Ybar_10_t3)))
    gamma6_hat=mean(X_vetor_1_inf[,q+2])-(exp((G_Ybar_01_t6-G_Ybar_00_t3+G_Ybar_10_t3)))
    ...
    gamma4_hat=mean(X_vetor_1_inf[,q])-(F_inver_F_Y10_t4)
    gamma5_hat=mean(X_vetor_1_inf[,q+1])-(F_inver_F_Y10_t5)
    gamma6_hat=mean(X_vetor_1_inf[,q+2])-(F_inver_F_Y10_t6)
