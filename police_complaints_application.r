...
for (p in sort(unique(dados$first_trained))[-1]){
  l_matriz=l_matriz+1
    ...
    dados_temp=dados[dados$first_trained<=t_interesse,]
    for (i in unique(dados_temp$uid)){
      ...
      while (dados_temp$uid[c]==i && c<=length(dados_temp$uid)) {
        ...
        if(dados_temp$period[c]==t_interesse_1) minha_base$COMPLAINTS_BEFORE[k]=minha_base$COMPLAINTS_BEFORE[k]+dados_temp$complaints[c]
